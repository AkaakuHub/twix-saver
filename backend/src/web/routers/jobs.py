...
async def create_job(job_data: ScrapingJobCreate, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not valid_users:
            raise HTTPException(status_code=400, detail="有効なユーザー名が指定されていません")
        ...
        logger.info(f"新しいスクレイピングジョブを作成: {job_id} (ターゲット: {', '.join(valid_users)})")
...
async def create_job_from_active_users(
    ...
    try:
        ...
        logger.info(
            f"アクティブユーザーからジョブを作成: {job_id} ({len(usernames)}ユーザー, 優先度{min_priority}以上)"
        )

        return SuccessResponse(
            message="アクティブユーザーから自動ジョブを作成しました",
            data={
                "job_id": job_id,
...
async def run_pending_jobs_now(background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if job_count == 0:
            return SuccessResponse(message="実行待ちのジョブはありません", data={"pending_jobs": 0})
        ...
        return SuccessResponse(
            ...
            data={"pending_jobs": job_count, "status": "started"},
        )
...
async def delete_job(job_id: str):
    ...
    try:
        ...
        if job.status in ["running"]:
            raise HTTPException(
                status_code=400,
                detail="実行中のジョブは削除できません。まず停止してください。",
            )
        ...
        if success:
            ...
            return SuccessResponse(message="ジョブを削除しました", data={"job_id": job_id})
        else:
